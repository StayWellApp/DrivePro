...
export default function IntelligencePane({ isVisible, onClose, fault }: IntelligencePaneProps) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-zinc-950/80 backdrop-blur-[30px] border-l border-white/10 z-[1000] shadow-2xl p-8 flex flex-col transform transition-transform duration-300">
      ...
      <div className="flex-1">
        <h3 className="text-zinc-500 uppercase tracking-widest text-xs font-bold mb-2">{t('analysis')}</h3>
        <h2 className="text-3xl font-black text-white mb-6 uppercase leading-tight">{fault.category}</h2>

        <div className="mb-8 p-6 bg-white/5 rounded-2xl border border-white/10">
          <p className="text-zinc-400 text-sm mb-2 font-medium uppercase tracking-wider">{t('instructorNotes')}</p>
        ...
        <div className="p-6 bg-white/5 rounded-2xl border border-white/10 flex items-center justify-between">
          <div>
            <p className="text-zinc-400 text-sm font-medium uppercase tracking-wider mb-1">{t('riskScore')}</p>
