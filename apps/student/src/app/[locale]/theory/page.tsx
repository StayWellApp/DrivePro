    ...
    const mockQuestions: Question[] = [
      {
        ...
        answer: "No entry",
        explanation:
          'The "No Entry" sign indicates that entry for all vehicles is prohibited. This is typically used at one-way streets and other areas where traffic flow must be restricted.',
      },
      {
        id: "2",
        ...
        explanation:
          "Rear fog lights should only be used when visibility is significantly reduced (less than 100 meters) to avoid dazzling other drivers.",
      },
      {
        id: "13",
        question:
          "When encountering this specific road marking, what is the mandatory action for a driver intending to turn left?",
        options: [
        ...
        answer:
          "Yield to oncoming traffic and only proceed when the way is clear.",
        explanation:
          'The "Yield" marking indicates that drivers must give right-of-way to other road users. Unlike a "Stop" sign, you are not required to stop if the road is clear, but you must be prepared to do so.',
      },
    ];
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center bg-surface-container-high px-4 py-2.5 rounded-xl gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <div
                  className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${studyMode ? "translate-x-5" : "translate-x-0"}`}
                ></div>
              </button>
            </div>
          </div>
          <button
            onClick={finishTest}
            className="bg-primary-container text-white px-6 py-2.5 rounded-xl font-bold hover:scale-95 transition-transform"
          >
            {t("submitExam")}
          </button>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-surface-container-lowest rounded-xl p-8 shadow-sm border border-outline-variant/10">
            {currentQuestion && (
              <>
                <div className="mb-8">
                  <span className="text-on-surface-variant text-xs font-bold tracking-widest uppercase block mb-4">
                    {t("questions")} {currentIndex + 1}
                  </span>
                ...
                {currentQuestion.id === "13" && (
                  <div className="aspect-video w-full rounded-xl overflow-hidden mb-10 bg-slate-100 group">
                    <img
                      ...
                      alt="Road marking"
                      src="https://lh3.googleusercontent.com/aida-public/AB6AXuAirfBXxDXZzsSaPLG2NYOUNcOyI9suzYMf7GW0pJtExA33G_1AYrxxDBCn7GntDtapOM2KbPE8da150tcuK9bKtylR15srhoSfD84Wd_ntg9x1dMWpSVKLu4C8z_7271Ry_o-VgdMogET7Z51x5AjETTvexrqRQSd7gxWSliP1-ddMpNF8Gko35tb77yxJ3-NWTIA8hCWEyedQ60YqlQQbf7meXLdgR0OrGFWl9DU1emm7NBq_p6f6PoSJOOCegtzG-NZ_UAFz9Q"
                    />
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 gap-4">
                  {currentQuestion.options.map((option, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-colors ${letterClass}`}
                        >
                          {letters[idx]}
                        </span>
                        <span
                          className={`flex-1 font-medium text-lg ${studyMode && feedback && isCorrect ? "text-teal-900 font-bold" : ""}`}
                        >
                          {option}
                        </span>
          ...
          {studyMode && feedback && currentQuestion && (
            <div className="glass-panel rounded-xl p-8 border-l-4 border-teal-400 bg-surface-container-lowest shadow-sm">
              ...
              <div className="flex gap-2">
                <span className="bg-surface-container px-3 py-1 rounded-md text-[10px] font-bold tracking-tighter uppercase text-outline">
                  Road Rules
                </span>
                <span className="bg-surface-container px-3 py-1 rounded-md text-[10px] font-bold tracking-tighter uppercase text-outline">
                  Difficulty: Medium
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-surface-container-low rounded-xl p-6 sticky top-24">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs bg-surface-container-highest px-2 py-1 rounded font-bold">
            ...
            <div className="grid grid-cols-5 gap-2">
              ...
              {[...Array(27)].map((_, i) => (
                <button
                  key={i}
                  className="h-10 w-full rounded-lg bg-surface-container-highest text-on-surface-variant text-xs font-bold flex items-center justify-center opacity-40 cursor-not-allowed"
                >
          ...
          <div className="bg-primary-container text-white rounded-xl p-6 relative overflow-hidden group shadow-lg">
            ...
            <h5 className="text-xs font-black uppercase tracking-[0.2em] mb-2 opacity-70">
              {t("proTip")}
            </h5>
            <p className="text-sm font-medium leading-relaxed relative z-10">
              "Always look twice for motorcyclists when turning right at
              junctions. Their speed can be deceptive."
            </p>
          </div>
        </div>
      </div>
