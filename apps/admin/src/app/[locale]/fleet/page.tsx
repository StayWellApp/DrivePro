            ...
            >
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  {status !== "ok" && (
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${isOverdue ? "bg-red-600 text-white" : "bg-amber-500 text-white"}`}
                    >
                      {t(
                        status === "warning"
                          ? "statusWarning"
              ...
              <div className="flex gap-4 mt-4 md:mt-0">
                <div
                  ...
                  ${
                    ...
                    (vehicle.nextServiceMileage &&
                      vehicle.nextServiceMileage - vehicle.current_mileage <= 0)
                ...
                >
                  ...
                  <span
                    className={`font-bold ${isOverdue ? "text-red-700" : isWarning ? "text-amber-700" : "text-zinc-700"}`}
                  >
                  ...
                  <span className="text-[10px] text-zinc-400 mt-1 uppercase font-bold tracking-tighter">
                    ...
                    {vehicle.nextServiceMileage || "∞"} km
                  </span>
                </div>

                <div
                  className={`p-4 rounded-lg border flex flex-col items-center ${status !== "ok" && vehicle.stk_expiry && new Date(vehicle.stk_expiry).getTime() - new Date().getTime() < 0 ? "bg-red-50 border-red-200" : "bg-zinc-50 border-zinc-100"}`}
                >
                  <span className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-1">
                    {t("stkExpiry")}
                  </span>
