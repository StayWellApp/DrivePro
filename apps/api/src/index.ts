...
app.post(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    console.log(
      `Uploaded document for vehicle ${vehicleId}: ${file.originalname}`,
    );
    ...
    return res
      ...
      .json({
        success: true,
        url: `https://storage.googleapis.com/drivepro-docs/${file.filename}`,
      });
  },
);
...
app.get(
  ...
  async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch theory questions" });
    }
  },
);
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    if (!school_id || !video_file_name || !video_start_timestamp) {
      return res
        ...
        .json({
          error:
            "school_id, video_file_name, and video_start_timestamp are required",
        });
    }
    ...
    try {
      ...
      const updatePromises = faultPins.map((pin) => {
        const offsetSeconds = Math.max(
          0,
          Math.floor((pin.timestamp.getTime() - videoStart.getTime()) / 1000),
        );
      ...
      return res
        .status(200)
        .json({ success: true, updatedPins: faultPins.length });
    } catch (error) {
...
app.get(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    try {
      ...
      const faults: Record<string, number> = faultPins.reduce(
        (acc: Record<string, number>, pin: any) => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=lesson-report-${lessonId}.pdf`,
      );
